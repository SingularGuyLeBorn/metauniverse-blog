...
符号含义类型说明$\theta$参数向量策略网络的参数$\alpha$学习率标量控制更新步长$\nabla_\theta$梯度算子计算损失对参数的偏导$\pi_\theta(as)$策略概率标量$G_t$累计回报标量$G_t = \sum_{k=0}^{\infty} \gamma^k r_{t+k}$**问题1：On-Policy限制**
...
符号含义说明$\frac{\pi_\theta(as)}{\pi_{\theta_{\text{old}}}(as)}$$A^{\pi_{\text{old}}}(s, a)$优势函数$A = Q(s,a) - V(s)$$D_{KL}$KL 散度衡量两个概率分布的差异$\delta$信任区域半径控制更新幅度**TRPO的问题**：
