...
**原因**：策略梯度公式中的期望 Eτ∼πθ[…]\mathbb{E}_{\tau \sim \pi_\theta}[\ldots]Eτ∼πθ​​[…] 要求轨迹必须来自**当前策略** πθ\pi_\thetaπθ​。
...
max⁡θEs∼ρπold[πθ(a∣s)πθold(a∣s)Aπold(s,a)]\max_\theta \quad \mathbb{E}_{s \sim \rho^{\pi_{\text{old}}}} \left[ \frac{\pi_\theta(a|s)}{\pi_{\theta_{\text{old}}}(a|s)} A^{\pi_{\text{old}}}(s, a) \right] θmax​Es∼ρπold​​[πθold​​(a∣s)πθ​(a∣s)​Aπold​(s,a)]
...
符号含义说明Ex∼p\mathbb{E}_{x \sim p}Ex∼p​在分布 ppp 下的**期望**我们想要计算的目标f(x)f(x)f(x)关于 xxx 的**任意函数**例如策略梯度中的目标Ex∼q\mathbb{E}_{x \sim q}Ex∼q​在分布 qqq 下的**期望**我们实际能采样的分布p(x)q(x)\frac{p(x)}{q(x)}q(x)p(x)​**重要性权重**校正采样分布的偏差p(x)p(x)p(x)xxx 在**目标分布** ppp 下的概率密度-q(x)q(x)q(x)xxx 在**采样分布** qqq 下的概率密度必须 q(x)>0q(x) > 0q(x)>0 当 p(x)>0p(x) > 0p(x)>0**推导**：
...
J(θ)=E(s,a)∼πθold[πθ(a∣s)πθold(a∣s)Aπθold(s,a)]J(\theta) = \mathbb{E}_{(s,a) \sim \pi_{\theta_{\text{old}}}}\left[\frac{\pi_\theta(a|s)}{\pi_{\theta_{\text{old}}}(a|s)} A^{\pi_{\theta_{\text{old}}}}(s, a)\right] J(θ)=E(s,a)∼πθold​​​[πθold​​(a∣s)πθ​(a∣s)​Aπθold​​(s,a)]
...
## 3. PPO-Clip目标函数 [​](#_3-ppo-clip目标函数)
